...
class BB84Protocol(NodeProtocol):
    ...
    def run(self):
        ...
        while True:
            if self.node.name == "Alice":
                ...
                yield self.await_timer(duration=2*self.delay)
                # Receive (RX) qubit on the port's input:
                print(f"{ns.sim_time():5.1f}: Alice: reading the port")
                ...
                if classical_message['type'] == 'bob_basis_confirm':
                    self.alice_confirm_basis(classical_message['content'])
                else: print('BOB BASIC CONFIRM: PROTOCOL ERROR!!!')
                yield self.await_timer(duration=2*self.delay)
                #yield self.await_port_input(self.node.ports["mixedIO"])
                message = self.node.ports["mixedIO"].rx_input()
                print(f"Alice: {message}")
                classical_message = message.items[0]
                if classical_message['type'] != 'end_cycle': print('BOB END CYCLE: PROTOCOL ERROR')
            else:
                # Wait (yield) until input has arrived on our port:
                yield self.await_port_input(self.node.ports["from_alice"])
                # Receive (RX) qubit on the port's input:
                message = self.node.ports["from_alice"].rx_input()
                print(f"{ns.sim_time():5.1f}: {message.items} received by {self.node.name}")
    ...
    def alice_begin_cycle(self):
        ...
        if self.bit == 0 and self.basis == "Z":
            ...
            qapi.operate(qubit, ns.X)
            qapi.operate(qubit, ns.H)
        self.node.ports["to_bob"].tx_output(qubit)
    
    def bob_receive_qubit(self, qubit):
        print(f"{ns.sim_time():5.1f}:===== BOB RECEIVE QUBIT =====")
        ...
        print(f'bob_receive_qubit: basis: {self.basis}')
    ...
    def alice_confirm_basis(self, basis):
        print(f"{ns.sim_time():5.1f}:===== ALICE CONFIRM BASIS =====")
        if basis == self.basis:
        ...
        print(f"alice_confirm_basis: {self.basis_confirm}")
        self.node.ports["mixedIO"].tx_output({
    ...
    def bob_end_cycle(self, basis_confirm):
        print(f"{ns.sim_time():5.1f}:===== BOB END CYCLE =====")
        if basis_confirm:
            self.mutual_bits.append(self.bit)
        self.node.ports["mixedIO"].tx_output({
            'type': 'end_cycle'
...
class BB84KeyExchange():
    ...
    def __init__(self, dist, ex_time, offset_time=1,loss_init=0.1,loss_length=0.9):
        ...
        #suppose that the speed of light in the fiber is 150.000(km/h)
        #delay=dist*20/3
        #suppose that the speed of light in the fiber is 200.000(km/h)
        delay=dist*5

        #noise_model = DephaseNoiseModel(dephase_rate=dephase_rate,time_independent=True)
        loss_model = FibreLossModel(p_loss_init=loss_init,p_loss_length=loss_length)

        q_channel_AB = QuantumChannel(name="qchannel_AtoB",
                                    length=dist/self.offset_dist,delay=delay,
...
#                                    models={"delay_model": delay_model})
        ...
        node_alice.add_ports(["to_bob"])
        node_bob.add_ports(["from_alice"])
        node_alice.ports["to_bob"].connect(q_channel_AB.ports["send"])
        node_bob.ports["from_alice"].connect(q_channel_AB.ports["recv"])

        self.alice_protocol = BB84Protocol(node_alice,delay)
        self.bob_protocol = BB84Protocol(node_bob,delay)
