...
l_argv = sys.argv
l_argv.pop(0)   # remove the file name
...
loss_length = float(l_argv[2])
loss_init = float(l_argv[1])
total_dist = int(l_argv[0])
bb84 = BB84KeyExchange(total_dist, 100*total_dist, MILSECOND, loss_init, loss_length)
key_rate =  bb84.run()*(SECOND/MILSECOND)/1000
print(f'BB84 key rate: {key_rate}')
fptr = open("result.txt",'a')
fptr.write(f"{key_rate}\n")
fptr.close()
