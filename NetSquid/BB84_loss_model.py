    ...
    def run(self):
        while True:
            if self.node.name == "Alice":
                ...
                # Wait for 2*delay, the expected time for receiving the response from Bob
                yield self.await_timer(duration=2*self.delay)
                # Receive (RX) qubit on the port's input:
                message = self.node.ports["mixedIO"].rx_input()
                if message is None: continue    # if there is no response from Bob, which means photon is lost, Alice begins the new cycle
...
class BB84KeyExchange():
    offset_dist = 1 # default unit of length in channels is km
